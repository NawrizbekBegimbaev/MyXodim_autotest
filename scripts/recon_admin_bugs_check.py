"""Проверка статуса BUG-001..004 в Admin UI: создание компании + список."""

from __future__ import annotations

import os
import time
import uuid
from pathlib import Path

from dotenv import load_dotenv
from playwright.sync_api import Page, sync_playwright

load_dotenv()

ADMIN_URL = os.getenv("ADMIN_URL", "https://dev-hub-admin.greatmall.uz")
ADMIN_PHONE = os.environ["SUPER_ADMIN_PHONE"]
ADMIN_PASS = os.environ["SUPER_ADMIN_PASSWORD"]

OUT = Path("recon")


def snap(page: Page, label: str) -> None:
    OUT.mkdir(exist_ok=True)
    (OUT / f"{label}_url.txt").write_text(page.url)
    (OUT / f"{label}_full.yaml").write_text(page.locator("body").aria_snapshot())


def main() -> None:
    suffix = uuid.uuid4().hex[:6]
    company_name = f"[E2E recon] check {suffix}"
    slug = f"e2e-check-{suffix}"
    inn = str(int(time.time()))[-9:]
    admin_phone_local = f"905{int(time.time()) % 1_000_000:06d}"
    pinfl = "".join(str((i * 3 + 1) % 10) for i in range(14))

    api_log: list[str] = []

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        ctx = browser.new_context(
            viewport={"width": 1440, "height": 900},
            locale="ru-RU",
            ignore_https_errors=True,
        )
        page = ctx.new_page()

        page.on(
            "response",
            lambda r: api_log.append(
                f"{r.status} {r.request.method} {r.url}\n  {r.text()[:300]}\n"
            )
            if "/api/v1/admin" in r.url
            else None,
        )

        # Логин
        page.goto(f"{ADMIN_URL}/login", wait_until="networkidle")
        page.get_by_role("textbox", name="Телефон").fill(ADMIN_PHONE)
        page.get_by_role("textbox", name="Пароль").fill(ADMIN_PASS)
        page.get_by_role("button", name="Войти").click()
        page.wait_for_url("**/dashboard", timeout=15_000)
        page.wait_for_timeout(2_000)
        snap(page, "check_dashboard")

        # BUG-004: список компаний
        page.goto(f"{ADMIN_URL}/tenants", wait_until="networkidle")
        page.wait_for_timeout(2_500)
        snap(page, "check_tenants_list")

        # BUG-002: ПИНФЛ обязателен на форме?
        page.goto(f"{ADMIN_URL}/tenants/new", wait_until="networkidle")
        page.wait_for_timeout(1_500)
        snap(page, "check_form")

        # BUG-001/003: создание с полностью заполненной формой
        page.get_by_role("textbox", name="Название компании *").fill(company_name)
        page.get_by_role("textbox", name="Slug *").fill(slug)
        page.get_by_role("textbox", name="ИНН").fill(inn)
        page.get_by_role("textbox", name="Имя *").fill("Тест")
        page.get_by_role("textbox", name="Фамилия *").fill("Реконов")
        page.get_by_role("textbox", name="Телефон *").fill(admin_phone_local)
        # ПИНФЛ оставляем пустым — проверим BUG-002 (обязателен на бэке без * на фронте)
        page.get_by_role("button", name="Создать").click()
        page.wait_for_load_state("networkidle")
        page.wait_for_timeout(3_000)
        snap(page, "check_after_submit")

        (OUT / "check_api_log.txt").write_text("\n".join(api_log))

        page.close()
        ctx.close()
        browser.close()

    print(f"Done. Try: {company_name}, slug={slug}")


if __name__ == "__main__":
    main()

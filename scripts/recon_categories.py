"""Recon /categories — форма создания, edit, delete + tree."""
...
def snap(page: Page, label: str) -> None:
    ...
    (OUT / f"cat_{label}_url.txt").write_text(page.url)
    (OUT / f"cat_{label}.yaml").write_text(page.locator("body").aria_snapshot())


def main() -> None:
    suffix = uuid.uuid4().hex[:6]
    title = f"[E2E] Категория {suffix}"
    ...
    with sync_playwright() as p:
        ...
        # Заполняем единственный textbox
        ...
        # Submit (Создать / Добавить / Сохранить)
        for btn in ("Добавить", "Создать", "Сохранить"):
            try:

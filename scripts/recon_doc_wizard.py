"""Recon полного wizard'а создания документа /documents/create."""
...
def snap(page: Page, label: str) -> None:
    ...
    (OUT / f"docw_{label}.yaml").write_text(page.locator("body").aria_snapshot())
...
def main() -> None:
    with sync_playwright() as p:
        ...
        snap(page, "01_step1_template")

        # Click первого шаблона
        templates = page.get_by_role("main").get_by_role("button").all()
        for b in templates[:10]:
            txt = (b.text_content() or "")[:80]
            if "INTERNAL" in txt or "PDF" in txt:
                b.click()
                break
        page.wait_for_timeout(1_000)
        snap(page, "02_template_selected")

        # Далее
        page.get_by_role("button", name="Далее", exact=True).click()
        page.wait_for_timeout(2_000)
        snap(page, "03_step2_route")

        # Если есть выбор маршрута — список кнопок
        page.get_by_role("button", name="Далее", exact=True).click()
        page.wait_for_timeout(2_000)
        snap(page, "04_step3_review")

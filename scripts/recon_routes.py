"""Recon /routes — конструктор маршрутов согласования."""
...
def snap(page: Page, label: str) -> None:
    ...
    (OUT / f"rt_{label}_url.txt").write_text(page.url)
    (OUT / f"rt_{label}.yaml").write_text(page.locator("body").aria_snapshot())


def main() -> None:
    title = f"[E2E] Маршрут {uuid.uuid4().hex[:6]}"
    ...
    with sync_playwright() as p:
        ...
        page.goto(f"{CLIENT_URL}/routes", wait_until="networkidle")
        page.wait_for_timeout(1_500)
        snap(page, "01_list")

        page.get_by_role("button", name="Создать маршрут").click()
        page.wait_for_timeout(2_500)
        snap(page, "02_create_form")
        print(f"URL after Create button: {page.url}")
        page.close()
        ctx.close()
        browser.close()
    print(f"Done: {title}")

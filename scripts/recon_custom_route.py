"""Recon: 'Свой маршрут' tab в step 2 wizard'а — можно ли построить
1-step маршрут с admin'ом как approver чтобы потом approve в inbox."""
...
def snap(page: Page, label: str) -> None:
    ...
    (OUT / f"croute_{label}.yaml").write_text(page.locator("body").aria_snapshot())
...
def main() -> None:
    with sync_playwright() as p:
        ...
        title = f"[E2E] CustomRoute {secrets.token_hex(3)}"
        ...
        page.get_by_role("textbox", name="Содержание").fill("custom route")
        page.wait_for_timeout(500)
        page.get_by_role("button", name="Далее", exact=True).click()
        page.wait_for_timeout(2_500)

        # Step 2: switch to "Свой маршрут"
        page.get_by_role("button", name="Свой маршрут", exact=True).click()
        page.wait_for_timeout(1_500)
        snap(page, "01_custom_tab")

        # List buttons & comboboxes after switch
        btns = page.get_by_role("main").get_by_role("button").all()
        print(f"Custom tab buttons: {len(btns)}")
        for i, b in enumerate(btns[:40]):
            txt = (b.text_content() or "")[:80]
            if txt.strip():
                print(f"  [{i}] {txt!r}")
        cbs = page.get_by_role("main").get_by_role("combobox").all()
        print(f"Comboboxes: {len(cbs)}")

"""Recon /roles — кастомные роли + права доступа."""
...
def snap(page: Page, label: str) -> None:
    ...
    (OUT / f"role_{label}_url.txt").write_text(page.url)
    (OUT / f"role_{label}.yaml").write_text(page.locator("body").aria_snapshot())


def main() -> None:
    title = f"[E2E] Role {uuid.uuid4().hex[:6]}"
    ...
    with sync_playwright() as p:
        ...
        page.get_by_role("button", name="Создать роль").click()
        page.wait_for_timeout(1_500)
        snap(page, "02_dialog")
        ...
        try:
            ...
            snap(page, "03_filled")
            for btn in ("Создать", "Сохранить", "Добавить"):
                try:
            ...
            (OUT / "role_err.txt").write_text(str(e))
